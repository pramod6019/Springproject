...
randompic[11] = "../admin-ifx/signin-banner12.jpg";
randompic[12] = "../admin-ifx/signin-banner13.jpg";
randompic[13] = "../admin-ifx/signin-banner14.jpg";
...
function SelectCompanyContact(id, name, indexid, compid, spanid)
{
    ...
    compstr = "<a href=../customer/customer-contact-list.jsp?contact_id=" + id + " target = _blank><b>" + name + " ("+ id +")</b></a>";
...
function SelectCompanyContact1(id, accid, accname, name, indexid, compid, spanid)
{
    ...
    var spancontid=window.parent.document.getElementById("span_cont_id");
	var spanidObj1=window.parent.document.getElementById("span_bill_customer_id");
    ...
    compstr = "<a href=../customer/customer-contact-list.jsp?contact_id=" + id + " target = _blank><b>" + name + " ("+ id +")</b></a>";
	compstr1 = "<a href=../customer/customer-list.jsp?customer_id=" + accid + " target = _blank><b>" + accname + " ("+ accid +")</b></a>";
...
{  
    ...
    compstr = "<a href=../customer/customer-list.jsp?customer_id=" + id + " target = _blank><b>" + name + " (" + id + ")</b></a>";		
    spanidObj.innerHTML = compstr;
    //alert(compid + " selected.");
	window.parent.$('#dialog-modal').dialog('close');
}
...
function SelectContractAsset1(id, name, indexid, compid, spanid)
{
	//alert("insideeeeeeeeeeee");
    var assid=window.parent.document.getElementById("asset_id").value; 
    var spanidObj=window.parent.document.getElementById("span_issue_asset_id");	
	//alert("spanidObj--"+spanidObj);
    var spanassid=window.parent.document.getElementById("span_asset_id");
	//alert("spanassid--"+spanassid);
...
{
    var accid=window.parent.document.getElementById("acc_id").value;
    var spanidObj=window.parent.document.getElementById("span_po_customer_id");
    var spanaccid=window.parent.document.getElementById("span_acc_id");
    var compstr;
    if(accid==id)
    {
...
function SelectReturnPO(id, indexid, compid, spanid)
{
    var poid=window.parent.document.getElementById("po_id").value;
    var spanidObj=window.parent.document.getElementById("span_return_po_id");
    var spanpoid=window.parent.document.getElementById("span_po_id");
    var compstr;
    if(poid==id)
    {
        spanpoid.value=poid;
    ...
    compstr = "<a href=../inventory/inventory-po-list.jsp?po_id=" + id + " target = _blank><b>" + id + "</b></a>";
    spanidObj.innerHTML = compstr;
    alert(compid + " selected.");
    window.parent.location = "../inventory/inventory-preturn-add.jsp?status=add&po_id="+id+""
}
...
function SelectCompany(id, name, indexid, companyid, spanid)
{
    ...
    var spanidObj=getObjectOpener(spanid);
    var companystr
    companystr = "<a href=customer-list.jsp?customer_id=" + id + " target = _blank><b>" + name + " (" + id + ")</b></a>";
    indexidObj.value = id;
...
function toInteger(textid)
{
    ...
    var newString = "";		 // REVISED/CORRECTED STRING
    var count = 0;
    //alert("hi--"+textidObj.value);// COUNTER FOR LOOPING THROUGH STRING
    // LOOP THROUGH STRING
    for (i = 0; i < textidObj.value.length; i++) 
    {
		//alert("hi--"+textidObj.value);
		//alert("ch---"+textidObj.value.substring(i, i+1));
        ch = textidObj.value.substring(i, i+1);  // CHECK EACH CHARACTER
		//alert("ch---"+ch);
        if ((ch >= "0" && ch <= "9")) 
        {
...
{
     // alert("hi====="+document.getElementById(id).style.display);
    var elem = document.getElementById(id);
